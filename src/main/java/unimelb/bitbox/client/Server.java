...
public class Server implements Runnable {
    ...
    private JsonDocument handleMessage(String message)
        ...
        try {
            ...
            ServerMain.log.info("Received " + document.toJson());

            command = document.require("command");
        } catch (CryptoException | ResponseFormatException e) {
            response.append("command", "AUTH_RESPONSE");
            response.append("status", false);
            response.append("message", "Failed to interpret message: " + e.getMessage());
            ServerMain.log.warning("Failed to interpret message: " + e.getMessage());
            return response;
        }
