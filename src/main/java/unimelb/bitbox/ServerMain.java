...
public class ServerMain implements FileSystemObserver {
    ...
    public PeerConnection getPeer(HostPort hostPort) {
        synchronized (peers) {
            for (PeerConnection peer : peers) {
                HostPort peerHostPort = peer.getHostPort();
                HostPort peerLocalHostPort = peer.getLocalHostPort();

                if (peerHostPort.fuzzyEquals(hostPort) || peerLocalHostPort.fuzzyEquals(hostPort)) {
                    return peer;
                }
            }
        }
    ...
    public PeerConnection tryPeer(HostPort target) {
	    if (target.fuzzyEquals(hostPort)) {
            ServerMain.log.warning("Asked to connect to myself");
	        return null;
        }
    ...
    public boolean clientTryPeer(HostPort hostPort) throws InterruptedException {
		if (getIncomingPeerCount() >= maxIncomingConnections) {
			return false;
		}
